...
def test_fat_in_middle():
    ...
    t = np.linspace(0, 2*np.pi, N)
    zs = np.linspace(zlo,zhi,N)
    r = (np.zeros(N)+r)*np.sin(np.linspace(0,np.pi,N))
    xs = r*np.cos(8*t)
    ys = r*np.sin(8*t)
...
def test_decreasing_spatial_f():
    ...
    zhi = 202-10
    zlo = -117+10+70
    N = 2100 #has to be a multiple of 3 because of the reversing below
    if PLOT:
        mpl.rcParams['legend.fontsize'] = 10
        fig = plt.figure()
        ax = fig.gca(projection='3d')
    ...
    zs = np.linspace(0, zhi-zlo, N)
    #r = (np.zeros(N)+r)*np.sin(np.linspace(0,np.pi,N))
    #f = np.tile(np.linspace(1,0,n_turns/N),n_turns)
    f = np.tile(np.linspace(0,1,N/n_turns),n_turns)
    xs = r*np.cos((f*t)[::-1])
    ys = r*np.sin((f*t)[::-1])
